...
{
	if ((player.GetPosition().x > obstacle.GetPosition().x) && (obstacle.jumpedOver == false) && obstacle.GetPosition().x > 0 && obstacle.runnerCollision == false)
	{
		obstacle.jumpedOver = true;
		score++;
	}
	if ((player.GetPosition().x > enemy.GetPosition().x) && (enemy.jumpedOver == false) && enemy.GetPosition().x > 0 && enemy.runnerCollision == false)
	{
